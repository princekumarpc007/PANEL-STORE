...
/* ==========================================================================
   PAGE-SPECIFIC ENGINE: MY-KEYS.JS
   ========================================================================== */
function showToast(msg, type = 'success') {
        ...
        window.copyKey = function(id) {
            ...
            NexusAudio.playCopy();
            navigator.clipboard.writeText(text).then(() => showToast('Key copied!')).catch(() => {
        ...
        function getValidityBadge(item) {
            const planText = (item.label || item.plan || "").toLowerCase();
            const purchaseDate = new Date(item.date).getTime();
            if (isNaN(purchaseDate)) return '';

            let durationDays = null;
            if (planText.includes("1 day") || planText.includes("daily") || planText.includes("24 hour")) durationDays = 1;
            else if (planText.includes("3 day")) durationDays = 3;
            else if (planText.includes("7 day") || planText.includes("weekly") || planText.includes("1 week")) durationDays = 7;
            else if (planText.includes("15 day")) durationDays = 15;
            else if (planText.includes("30 day") || planText.includes("monthly") || planText.includes("1 month")) durationDays = 30;
            else if (planText.includes("60 day") || planText.includes("2 month")) durationDays = 60;
            else if (planText.includes("90 day") || planText.includes("3 month")) durationDays = 90;
            else if (planText.includes("year") || planText.includes("365")) durationDays = 365;
            else if (planText.includes("life") || planText.includes("perm")) {
                return `<span class="inline-flex items-center gap-1 text-[9px] font-mono px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/30"><i class="fas fa-infinity"></i> Lifetime</span>`;
            }

            if (!durationDays) {
                return `<span class="inline-flex items-center gap-1 text-[9px] font-mono px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/30"><i class="fas fa-shield"></i> Active</span>`;
            }

            const expiryTime = purchaseDate + (durationDays * 24 * 60 * 60 * 1000);
            const now = Date.now();
            const diff = expiryTime - now;

            if (diff <= 0) {
                return `<span class="inline-flex items-center gap-1 text-[9px] font-mono px-2 py-0.5 rounded bg-rose-500/15 text-rose-400 border border-rose-500/30 font-bold"><i class="fas fa-clock-rotate-left"></i> Expired</span>`;
            }

            const daysLeft = Math.floor(diff / (24 * 60 * 60 * 1000));
            const hoursLeft = Math.floor((diff % (24 * 60 * 60 * 1000)) / (60 * 60 * 1000));
            const label = daysLeft > 0 ? `${daysLeft}d ${hoursLeft}h left` : `${hoursLeft}h left`;
            return `<span class="inline-flex items-center gap-1 text-[9px] font-mono px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 font-bold"><i class="fas fa-hourglass-half animate-pulse"></i> ${label}</span>`;
        }
        ...
        onAuthStateChanged(auth, (user) => {
            ...
            onValue(ref(db, `purchases/${user.uid}`), (snap) => {
                if (!snap.exists()) {
                    container.innerHTML = `
                        <div class="premium-glass-card p-8 text-center">
                            <i class="fas fa-inbox text-gray-600 text-4xl mb-4"></i>
                            <p class="text-xs font-mono tracking-widest text-gray-500">No keys purchased yet</p>
                            <a href='./shop.html' class="inline-block mt-4 bg-purple-600 hover:bg-purple-500 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition shadow-[0_0_15px_rgba(147,51,234,0.4)]">Browse Store</a>
                        </div>`;
                    countEl.innerText = '';
                    return;
                }
                ...
                countEl.innerText = keys.length + ' key' + (keys.length > 1 ? 's' : '');

                let html = '';
                keys.forEach((item, i) => {
                    const d = new Date(item.date);
                    const dateStr = `${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`;
                    const planName = item.label || item.plan || 'Premium';
                    const licenseKey = item.key || 'N/A';
                    const link = item.link || '#';
                    const validityBadge = getValidityBadge(item);
                    const isExpired = validityBadge.includes("Expired");

                    html += `
                        <div class="key-card" style="animation-delay:${i * 0.08}s">
                            <div class="flex items-start justify-between gap-4 mb-4">
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <h4 class="text-sm font-black text-white uppercase tracking-wide truncate">${item.panelName || 'Nexus Module'}</h4>
                                        ${validityBadge}
                                    </div>
                                    <p class="text-[10px] text-gray-400 font-mono mt-1.5">
                                        <i class="fas fa-tag text-purple-500 mr-1"></i> ${planName}
                                        <span class="text-gray-700 mx-1.5">|</span>
                                        <i class="far fa-calendar text-gray-600 mr-1"></i> ${dateStr}
                                    </p>
                                </div>
                                <div class="flex items-center gap-2 shrink-0">
                                    ${isExpired ? `<a href="./shop.html" class="px-2.5 py-1 bg-rose-600/20 text-rose-400 border border-rose-500/40 rounded-lg text-[9px] font-black uppercase hover:bg-rose-600 hover:text-white transition">Renew</a>` : ''}
                                    <span class="bg-purple-600/10 text-purple-400 border border-purple-500/30 px-3 py-1.5 rounded-lg text-[10px] font-black">₹${parseFloat(item.price || 0).toFixed(2)}</span>
                                </div>
                            </div>
                            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                                <div class="key-display group">
                                    <div class="flex items-center gap-3 overflow-hidden">
                                        <i class="fas fa-lock text-gray-600 group-hover:text-emerald-500 transition-colors text-sm shrink-0"></i>
                                        <span class="key-text truncate" id="key-${item.id}">${licenseKey}</span>
                                    </div>
                                    <button onclick="copyKey('key-${item.id}')" class="btn-copy shrink-0" title="Copy Key">
                                        <i class="fas fa-copy text-lg"></i>
                                    </button>
                                </div>
                                <button onclick="NexusAudio.playClick(); window.open('${link}', '_blank')" class="btn-download">
